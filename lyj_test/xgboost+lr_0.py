...
import sys
import datetime
def xgb_feature_encode(libsvmFileNameInitial, xgbfeaturefile):
    # load样本数据
    X_all, y_all = load_svmlight_file(libsvmFileNameInitial)
    # 训练/测试数据分割
    X_train, X_test, y_train, y_test = train_test_split(X_all, y_all, test_size = 0.3, random_state = 42)
    # 定义模型
    xgboost = xgb.XGBClassifier(nthread=30, learning_rate=0.08,
                            n_estimators=200, max_depth=6, gamma=0, subsample=0.9, colsample_bytree=0.5)
    eval_set=[(X_test,y_test,'eval'),(X_train,y_train,'train')]
    # 训练学习
    xgboost.fit(X_train, y_train, early_stopping_rounds=50, eval_metric='auc',
                eval_set=eval_set, verbose=True)
    joblib.dump(xgboost, model_path + '/xgboost.m')
 ...
 #   X_leaves = np.concatenate((X_train_leaves, X_test_leaves), axis=0)
    X_leaves = X_test_leaves.astype(np.int32)
    (rows, cols) = X_leaves.shape
    # 记录每棵树的编码区间， cum_count值是每棵树的叶子节点数，并累加
    ...
    # 对所有特征进行ont-hot编码, x_leaves值代表总共树的第几个叶子节点，keyMapDict的key是之前模型的叶子节点编码，value是x_leaves
    for j in range(cols):
        keyMapDict = {}
        if j == 0:
            initial_index = 1
...
# 基于编码后的特征，将特征处理为libsvm格式且写入文件
    ...
    xgbFeatureLibsvm = open(xgbfeaturefile, 'w')
    for i in range(rows):
        xgbFeatureLibsvm.write(str(y_test[i]))
        for j in range(cols):
...
def xgboost_lr_train(xgbfeaturefile, origin_libsvm_file, model_path):
...
'''   X_train_ext = hstack([X_train_origin, X_train])
    ...
    joblib.dump(lr, model_path+'/lr_ext.m')

    # 预测及AUC评测
    y_pred_train=lr.predict_proba(X_train_ext)[:,1]
    lr_train_auc=roc_auc_score(y_train, y_pred_train)
    print("基于组合特征的LR train AUC: %.5f" % lr_train_auc)

    y_pred_test = lr.predict_proba(X_test_ext)[:, 1]
    lr_test_auc = roc_auc_score(y_test, y_pred_test)
    print('基于组合特征的LR test AUC: %.5f' % lr_test_auc)    '''
if __name__ == '__main__':
    if len(sys.argv) < 4:
        print(globals()['__doc__'] % locals())
        sys.exit(1)
    origin_libsvm_file, xgbfeaturefile, model_path = sys.argv[1:4]
    xgb_feature_encode(origin_libsvm_file, xgbfeaturefile)
    xgboost_lr_train(xgbfeaturefile, origin_libsvm_file, model_path)